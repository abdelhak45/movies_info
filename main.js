...
// document.getElementById('img').src = jsonApi.movies[2].poster_path
// console.log(jsonApi.movies[1])

let parent =document.getElementById('parent')

jsonApi.movies.map((e)=>{
    parent.innerHTML += `
    <div class="cards" style="width: 500px;height: fit-content;border: 1px solid gray;border-radius: 10px;padding: 10px;color: gray;">
    <img loading="lazy" src="${e.poster_path}" width=" 100%" height=" 250">
    <h1 style="text-align: center;" >${e.original_title}</h1>
    <span style="">
        <h3>Genres:</h3>
        <span id="type" style="color: slateblue;"><h6>${e.genres}</h6></span>
    </span>
    <h3>Overview:</h3>
...
${jsonApi.movies.map((el) => 
    
    `<option> ${el.original_title} </option>`
    )
}
`

function getChara(e) {
  jsonApi.movies.filter((elem)=>{
    if (e === elem.original_title) {
      ...
      <span style="">
          <h3>Genres:</h3>
          <span id="type" style="color: slateblue;"><h6>${elem.genres}</h6></span>
      </span>
      <h3>Overview:</h3>
